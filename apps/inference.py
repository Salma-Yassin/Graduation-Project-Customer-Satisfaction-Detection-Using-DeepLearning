...
def process_images(context_norm, body_norm, image_context_path=None, image_context=None, image_body=None, bbox=None):
    ...
    if image_context is None and image_context_path is None:
        raise ValueError(
            'both image_context and image_context_path cannot be none. Please specify one of the two.')
    if image_body is None and bbox is None:
        raise ValueError(
            'both body image and bounding box cannot be none. Please specify one of the two')

    if image_context_path is not None:
        image_context = cv2.cvtColor(cv2.imread(
            image_context_path), cv2.COLOR_BGR2RGB)
...
def infer(context_norm, body_norm, ind2cat, ind2vad, device, thresholds, models, image_context_path=None, image_context=None, image_body=None, bbox=None, to_print=True):
    ...
    image_context, image_body = process_images(
        context_norm, body_norm, image_context_path=image_context_path, image_context=image_context, image_body=image_body, bbox=bbox)
    ...
    for i in range(len(bool_cat_pred)):
        if bool_cat_pred[i] == True:
            ...
            if ind2cat[i] == "Annoyance":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Anticipation":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Aversion":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Confidence":
                pos_counter = pos_counter+1
            if ind2cat[i] == "Disapproval":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Disconnection":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Disquietment":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Doubt/Confusion":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Embarrassment":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Engagement":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Esteem":
                pos_counter = pos_counter+1
            if ind2cat[i] == "Excitement":
                pos_counter = pos_counter+1
            if ind2cat[i] == "Fatigue":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Fear":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Happiness":
                pos_counter = pos_counter+1
            if ind2cat[i] == "Pain":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Peace":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Pleasure":
                pos_counter = pos_counter+1
            if ind2cat[i] == "Sadness":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Sensitivity":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Suffering":
                neg_counter = neg_counter+1
            if ind2cat[i] == "Surprise":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Sympathy":
                neu_counter = neu_counter+1
            if ind2cat[i] == "Yearning":
                neu_counter = neu_counter+1
    emotion = list()
    if neg_counter > pos_counter and neg_counter > neu_counter:
        emotion.append("Negative")
    elif pos_counter > neg_counter and pos_counter > neu_counter:
...
def inference_emotic(images_list, model_path, result_path, context_norm, body_norm, ind2cat, ind2vad, args):
    ...
    thresholds = torch.FloatTensor(
        np.load(os.path.join(result_path, 'val_thresholds.npy'))).to(device)
    model_context = torch.load(os.path.join(
        model_path, 'model_context1.pth')).to(device)
    model_body = torch.load(os.path.join(
        model_path, 'model_body1.pth')).to(device)
    emotic_model = torch.load(os.path.join(
        model_path, 'model_emotic1.pth')).to(device)
    model_context.eval()
    model_body.eval()
    emotic_model.eval()
    ...
    for idx, line in enumerate(lines):
        ...
        pred_cat, pred_cont = infer(context_norm, body_norm, ind2cat, ind2vad, device,
                                    thresholds, models, image_context_path=image_context_path, bbox=bbox)
